...
public:
	...
	Water(HWND hWnd, OpenGLClass* OpenGL, TextureClass* LoaderTextureReference, const wchar_t* mapaDeAlturaHeightmap,
		string indiceTextura1, string indiceTextura2, string indiceTextura3, float anchoDeseado, float profundidadDeseada)
	{
		worldMatriz = new float[16] {0.0f};
		OpenGL->BuildIdentityMatrix(worldMatriz);
		...
		this->Plano(Ancho(), Alto(), anchoDeseado, profundidadDeseada, Dir_Imagen(), 80);
	...
	void Render(OpenGLClass* OpenGL, float* viewMatrix, float* projectionMatrix) {
		// Seleccionar el shader para el dibujo del terreno
		LandShader->SetShader();
		LandShader->PonMatriz4x4((char*)"worldMatrix", worldMatriz);
	...
	void animateWater(OpenGLClass* OpenGL) {
		if (yMove < 1 && goUp) {
			yMove += 0.01;
		}
		else if (yMove >= 1 && goUp) {
			goUp = false;
		}
		else if (yMove >= -0.5 && !goUp) {
			yMove -= 0.01;
		}
		else {
			goUp = true;
		...
		else if (zMove >= 1 && goUp) {
			goLeft = false;
		}
		else if (zMove >= -0.5 && !goUp) {
			zMove -= 0.01;
		}
		else {
			goLeft = true;
		}
		OpenGL->MatrixTranslation(worldMatriz, 0, 0 + yMove, 0 + zMove);
	}
};
